...
head(nucleosomeMap$wp.reads, n = 3)
...
reads). Then, reads are splitted into forward and reverse reads of fixed length
(`read.len`). Those reads are stored in a `data.frame` containing the name of
a synthetic chromosome (`chr_SYNTHETIC`), the starting position, the ending
...
nucleosomeSample <- syntheticNucReadsFromDist(wp.num=wp.num, wp.del=wp.del,
                                        ...
                                        fuz.var=fuz.var, max.cover=max.cover,
                                        nuc.len=nuc.len, len.var=len.var,
                                        read.len=read.len, lin.len=lin.len, 
                                        rnd.seed=rnd.seed,
...
head(nucleosomeSample$wp, n = 4)

#### The central position of all fuzzy nucleosomes with the
#### number of paired-end reads each associated to each one
head(nucleosomeSample$fuz, n = 4)
...
#### reverse reads
head(nucleosomeSample$dataIP, n = 4)
```
