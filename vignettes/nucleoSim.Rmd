---
title: "nucleoSim: Generate synthetic nucleosome maps"
output:
...
sequences covering nucleosome regions as well as a synthetic maps with 
...
from the beginning of each section (`round(nuc.len/2)` bases). Sequencess are 
...
are deleted. A nucleosome is considered deleted when all sequences associated 
...
max.cover        <- 70         ### Maximum sequences associated with one nucleosome (default: 100)
nuc.len          <- 147        ### Length of the nucleosome (default: 147)
len.var          <- 12         ### variance associated with the length of the sequences (default: 10)
...
#### A IRanges object containing all sequences for the well-positioned nucleosomes
...
#### Control sequences for hybridization data (only available when as.ratio is TRUE)
...
Following an uniform distribution, a fixed number of nucleosomes (`wp.del`) 
are deleted. A nucleosome is considered deleted when all paired-end reads 
