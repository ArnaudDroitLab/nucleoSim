...
start position of the reads associated to the nucleosomes
...
A synthetic genome is splitted in `wp.num` sections of `(nuc.len + lin.len)` 
bases. The nucleosomes are positioned at `round(nuc.len/2)` bases from the 
beginning of each section. A number of reads is assigned to each
...
val.num       <- 20  ### nombre de nucleosomes non-chevauchants
val.del       <- 5
...
sample <- syntheticNucMapFromDist(wp.num=val.num,
                                            wp.del=val.del,
                                            wp.var=val.var,
                                            fuz.num=val.fuz.num,
                                            fuz.var=val.fuz.var,
...
val.num       <- 30  ### nombre de nucleosomes non-chevauchants
val.del       <- 10
val.var       <- 30   ### variance si autre dist
val.fuz.num   <- 10
val.fuz.var   <- 50   ### variance si sautre
