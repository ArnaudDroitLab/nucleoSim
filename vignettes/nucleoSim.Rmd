...
more realistic and to enable the evaluation of the influence of common 
'noise' on the detection of nucleosomes.
...
start position to the reads associated with the nucleosomes
* Select the variance associated with the distribution of the start position 
of the reads for the well-positioned 
nucleosomes
* Select the variance associated with the distribution of the start position 
of the reads for the fuzzy
nucleosomes
* Select the variance associated with the Normal distribution used to assign
the length of each read
* Select the maximum read coverage associated with one nucleosome
...
The `r Rpackage("nucleoSim")` package can generate a synthetic map with 
reads covering the nucleosome regions as well as a synthetic map with 
forward and reverse reads emulating next-generation sequencing. Finally,
synthetic hybridization data of "Tiling Arrays" can also be generated.

The `r Rpackage("nucleoSim")` package has been largely inspired by the
*Generating synthetic maps* section of the 
Bioconductor `r Biocpkg("nucleR")` package.
...
length of the reads corresponds to the length of the nucleosomes (`nuc.len`).

**2. Deleting some well-positioned nucleosomes**

Following an uniform distribution, a fixed number of nucleosomes (`wp.del`) 
are deleted. A nucleosome is considered deleted when all reads associated 
...
wp.var           <- 30         ### variance associated with the well-positioned nucleosomes
fuz.num          <- 5          ### Number of fuzzy nucleosomes
fuz.var          <- 50         ### Variance associated with the fuzzy nucleosomes
max.cover        <- 70         ### Maximum reads associated with one nucleosome (default: 100)
nuc.len          <- 147        ### Length of the nucleosome (default: 147)
len.var          <- 12         ### variance associated with the length of the reads (default: 10)
...
#### The number of reads associated with each well-positioned nucleosome
...
#### The number of reads associated with each fuzzy nucleosome
nucleosomeMap$fuz.nreads

#### A IRanges object containing all reads for the fuzzy nucleosomes
head(nucleosomeMap$fuz.reads, n = 2)

#### A IRanges object containing all reads from well-positioned and fuzzy nucleosomes
...
`   (x,y) = 
(the central position of the nucleosome, the number of reads associated with
the nucleosome)`
...
a control map of random reads created using a uniform distribution. The
...
#### Control reads for hybridization data (only available when as.ratio is TRUE)
head(nucleosomeMapTiling$ctr.reads, n=4)

#### Ratio for hybridization data (only available when as.ratio is TRUE)
...
is created using the same first 3 steps than the 
...
The reads are split into forward and reverse reads of fixed length
...
This function needs information about the nucleosomes and their distribution to 
...
fuz.var          <- 50            ### Variance associated with the fuzzy nucleosomes
max.cover        <- 90            ### Maximum reads associated with one nucleosome (default: 100)
nuc.len          <- 147           ### Length of the nucleosome (default: 147)
len.var          <- 12            ### variance associated with the length of the reads (default: 10)
lin.len          <- 20            ### Length of the DNA linker (default: 20)
...
rnd.seed         <- 202           ### Set seed when result needs to be reproducible
