...
synthetic hybridation data of "Tiling Arrays" can also be generated.
...
### Simulate hybridation data of Tiling Arrays

The `syntheticNucMapFromDist()` function contains an option (`as.ratio`) which
unable the simulation of hybridation data of "Tiling Arrays". The data are
...
as.ratio         <- TRUE       ### Activate the simulation of hybridation data
rnd.seed         <- 212309     ### Set seed when result need to be reproducible
        
#### Create a synthetic nucleosome map with simulation of hybridation data
...
#### Control reads for hybridation data (only available when as.ratio is TRUE)
head(nucleosomeMapTiling$ctr.reads, n=4)

#### Ratio for hybridation data (only available when as.ratio is TRUE)
